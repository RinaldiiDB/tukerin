...
@section('title', 'Log Transaksi Penukaran')

@section('content')
<div class="bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Log Transaksi Penukaran</h1>
            <p class="text-xs text-slate-500 font-medium mt-1">Daftar rekaman seluruh transaksi masuknya botol daur ulang di sistem</p>
        </div>
        <div>
        ...
        <table class="min-w-full divide-y divide-slate-100 text-left text-sm">
            <thead class="bg-slate-50 text-slate-500 font-bold text-xs uppercase tracking-wider">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-4">Petugas Pegawai</th>
                    <th scope="col" class="px-6 py-4">Rincian Item</th>
                    <th scope="col" class="px-6 py-4 text-right">Poin</th>
                </tr>
            </thead>
                ...
                @forelse($transactions as $tx)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <span class="text-[10px] text-slate-400 block mt-0.5">{{ $tx->user->profile ? $tx->user->profile->phone : '-' }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            {{ $tx->employee->name }}
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-1 max-w-md">
                                @foreach($tx->details as $d)
                                    <span class="inline-block bg-slate-100 border border-slate-200/30 px-2 py-0.5 rounded text-[10px] text-slate-600 font-semibold">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center">
                            ...
                            <p class="text-xs text-slate-400 font-medium">Belum ada transaksi terekam.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
