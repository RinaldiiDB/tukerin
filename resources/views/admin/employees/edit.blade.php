...
<div class="max-w-md mx-auto">
    <div class="bg-white border border-slate-200/60 rounded-3xl p-6 md:p-8 shadow-xl">
        <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Data Pegawai</h1>
        <p class="text-xs text-slate-500 font-medium mt-1">Ubah informasi akun untuk operator lapangan.</p>

        <form action="{{ route('admin.employees.update', $employee->id) }}" method="POST" class="space-y-4 mt-6">
            @csrf
            @method('PUT')
            ...
            <!-- Password -->
            <div>
                <label for="password" class="block text-sm font-semibold text-slate-700 font-medium">Password Baru (Opsional)</label>
                <div class="mt-1">
